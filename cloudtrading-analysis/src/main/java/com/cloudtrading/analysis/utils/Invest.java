...
public class Invest {
	private static final Logger logger = LoggerFactory.getLogger(Invest.class);
	...
	private synchronized Result dealLastCalculate(int position,Date date,Long valueTime){
		...
		if(this.createDirection==Direction.INSCREASE){  //买涨
			...
			if(this.createPosition-position>=this.destination){ //涨 ：  买点-现在的大小>止盈止损点
				result=new Result();
				result.setCREATE_POSITION(this.createPosition);
				result.setPOSITION(position);
				result.setDATE(date);
				result.setIS_SUCESS(0);
	...
	public Invest(int production,int marginalValue,int destination,int ifFailureMarginalValue){
		this.maxPosition=0;
		this.minPosition=0;
		this.lastPosition=0;
	...
	private  Warehouse create(int position,Direction direction,long valueTime) throws AWTException {
		...
		Warehouse  warehouse=null;
		float allK=0;
		allK=(float) (((position-this.lastPosition)*1000000))/(valueTime-this.lastTime)/100000;
		String kString="斜率="+allK;
		float k=0;
		k=(float) (((position-this.tapeStartPoint)*1000000))/(valueTime-this.tapeStartTime)/100000;
		...
		if(direction==Direction.INSCREASE){
			if(this.failureInscreseCount>marginalValue&&this.failureInscreseCount<=this.marginalValue+this.ifFailureMarginalValue){  //不贪，只买 6 7两次
				...
				warehouse.setDESCRIPTION(kString+"\t"+"开始值"+this.tapeStartPoint+"大盘变化斜率:"+k+"     第一次亏了！！这次我需要买跌"+"在"+position+"点我买了跌"+DateFormatUtil.formatSimpleDate(new Date(valueTime)));
				createCount++;
				//TODO 调用机器买跌,做种类判断
				...
				warehouse.setCREATE_TIME(date);
				warehouse.setDESCRIPTION(kString+"\t"+"开始值"+this.tapeStartPoint+"大盘变化斜率:"+k+"这次我真买："+"在"+position+"点我买了涨"+DateFormatUtil.formatSimpleDate(new Date(valueTime)));
				//TODO 调用机器买涨
				if(this.isCreate){
				...
				warehouse.setCREATE_TIME(date);
				warehouse.setDESCRIPTION(kString+"\t"+"开始值"+this.tapeStartPoint+"大盘变化斜率:"+k+"   "+"在"+position+"我不敢买"+DateFormatUtil.formatSimpleDate(new Date(valueTime)));
			}else{
				warehouse=new Warehouse();
				Date date=new Date();
				warehouse.setCREATEPOSITION(position);
				...
				warehouse.setDIRECTION(1);
				warehouse.setCREATE_TIME(date);
				warehouse.setDESCRIPTION(kString+"\t"+"在"+position+"点我买了涨"+DateFormatUtil.formatSimpleDate(new Date(valueTime)));
			...
			if(this.failureDescreseCount>this.marginalValue&&this.failureDescreseCount<=this.marginalValue+this.ifFailureMarginalValue){  //不贪，只买 6 7两次
				...
				warehouse.setDIRECTION(1);
				warehouse.setCREATE_TIME(date);
				warehouse.setDESCRIPTION(kString+"\t"+"开始值"+this.tapeStartPoint+"大盘变化斜率:"+k+"     第一次亏了！！这次需要买涨"+"在"+position+"点我买涨"+DateFormatUtil.formatSimpleDate(new Date(valueTime)));
				createCount++;
				//TODO 调用机器买跌
				...
				warehouse.setDIRECTION(1);
				warehouse.setCREATE_TIME(date);
				warehouse.setDESCRIPTION(kString+"\t"+"开始值"+this.tapeStartPoint+"大盘变化斜率:"+k+"这次我真买："+"在"+position+"点我买了跌"+DateFormatUtil.formatSimpleDate(new Date(valueTime)));
				createCount++;
				//TODO 调用机器买跌
				if(this.isCreate){
					//WindowsController.submit();
				...
				warehouse.setDIRECTION(2);
				warehouse.setCREATE_TIME(date);
				warehouse.setDESCRIPTION(kString+"\t"+"在"+position+"点我买了跌"+DateFormatUtil.formatSimpleDate(new Date(valueTime)));
				//此时为模拟买
			}
		}
		return warehouse;
