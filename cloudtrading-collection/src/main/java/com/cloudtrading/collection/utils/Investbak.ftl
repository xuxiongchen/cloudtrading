...
import com.cloudtrading.collection.entity.Result;
import com.cloudtrading.collection.entity.Warehouse;
import com.cloudtrading.collection.entity.WarehouseAndLastResult;
...
public class Invest {
	 ...
	 * @param position
	 */
	private Result dealLastCalculate(int position,Date date,Long valueTime){
		Result result=null;
	...
	public WarehouseAndLastResult readPosition(int position,long valueTime,Date date) throws AWTException{
		...
		//处理建仓
		Warehouse warehouse=this.createWarehouse(position, date,valueTime);
	 ...
	 * @throws AWTException 
	 */
	private Warehouse createWarehouse(int position,Date date,long valueTime) throws AWTException{
	...
	private  Warehouse create(int position,Direction direction,long valueTime) throws AWTException {
		...
		if(this.failureInscreseCount==this.marginalValue-1){
			//准备买
			WindowsController.inscreseGoldCopperReady();
			isPrepreForWhat=1;
		}else if(this.failureDescreseCount==this.marginalValue-1){
			WindowsController.descreseGoldCopperReady();
			isPrepreForWhat=2;
		}
